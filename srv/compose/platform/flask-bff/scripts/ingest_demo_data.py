...
def _ensure_correspondence_table(connection) -> None:
    with connection.cursor() as cursor:
        cursor.execute("SELECT to_regclass(%s)", ("platform.correspondence",))
        result = cursor.fetchone()
        if not result or result[0] is None:
            raise DemoDataError(
                "Missing platform.correspondence table. Apply 006_platform_correspondence.sql before ingesting."
            )
...
def _load_reference_payload(
    ...
    *,
    ...
    if not ref_path.exists():
        raise DemoDataError(f"Referenced payload not found: {reference} (from {source_path.name})")
    payloads = parse_demo_payload(ref_path)
    if len(payloads) == 1:
        return payloads[0]
    return payloads
...
def ingest_demo_data(
    ...
    with _get_connection() as connection:
        ...
        if any(correspondence_re.match(path.name) for path in files):
            _ensure_correspondence_table(connection)
        with connection.cursor() as cursor:
            for path in files:
                contract_name = resolve_contract(path.name, default_contract)
                ...
                if correspondence_match:
                    ...
                    for index, item in enumerate(payload_items):
                        payload_value: dict | list
                        if "config_ref" in item:
                            ref_value = item.get("config_ref")
                            if not isinstance(ref_value, str) or not ref_value:
                                raise DemoDataError(f"Invalid config_ref in {path.name}")
                            loaded = _load_reference_payload(data_dir, ref_value, source_path=path)
                            payload_value = {
                                "config_ref": ref_value,
                                "config": loaded,
                                "meta": {k: v for k, v in item.items() if k != "config_ref"},
                            }
                        elif "payload" in item:
                            payload_value = item.get("payload")
                            if not isinstance(payload_value, (dict, list)):
                                raise DemoDataError(f"Invalid payload entry in {path.name}")
                        else:
                            payload_value = item
                        cursor.execute(
                            """
                            INSERT INTO platform.correspondence
                                (msn_id, source_file, entry_index, payload)
                            VALUES (%s, %s, %s, %s)
                            """,
                            (msn_id, path.name, index, Json(payload_value)),
                        )
